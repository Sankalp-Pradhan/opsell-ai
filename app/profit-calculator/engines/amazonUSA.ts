...
interface TierBracket { from?: number; upTo: number; rate: number; }
interface ReferralFeeResult { referralFee: number; closingFee: number; }
...
interface SettingsInput { fulfillmentMethod?: 'Self-Ship' | 'Platform Fulfillment'; }
...
function computeReferralFee(category: string, price: number): ReferralFeeResult {
  const cat = (category || 'Everything Else').trim();

  function tiered(brackets: TierBracket[]): number {
    let fee = 0, remaining = price;
  ...
  let fee = 0, minFee = 0.3, closingFee = 0;

  switch (cat) {
    case 'Amazon Device Accessories': fee = price * 0.45; break;
    case 'Amazon Handmade': fee = price * 0.15; minFee = 1; break;
    case 'Apparel & Accessories':
      fee = tiered([{ from: 0, upTo: 15, rate: 0.05 }, { from: 15, upTo: 20, rate: 0.1 }, { from: 20, upTo: Infinity, rate: 0.17 }]); break;
    case 'Appliances - Compact': case 'Appliances – Compact':
      fee = tiered([{ from: 0, upTo: 300, rate: 0.15 }, { from: 300, upTo: Infinity, rate: 0.08 }]); break;
    case 'Automotive & Powersports': fee = price * 0.12; break;
    case 'Baby Products': case 'Beauty & Personal Care':
      fee = price <= 10 ? price * 0.08 : price * 0.15; break;
    case 'Books': case 'Music': case 'Software & Computer Video Games': case 'Video & DVD':
      fee = price * 0.15; closingFee = 1.8; break;
    case 'Camera & Photo': case 'Consumer Electronics': case 'Video Game Consoles':
      fee = price * 0.08; break;
    case 'Electronics Accessories':
      fee = tiered([{ from: 0, upTo: 100, rate: 0.15 }, { from: 100, upTo: Infinity, rate: 0.08 }]); break;
    case 'Grocery & Gourmet Food':
      fee = price <= 15 ? price * 0.08 : price * 0.15; minFee = 0; break;
    case 'Indoor & Outdoor Furniture':
      fee = tiered([{ from: 0, upTo: 200, rate: 0.15 }, { from: 200, upTo: Infinity, rate: 0.1 }]); break;
    case 'Jewelry':
      fee = tiered([{ from: 0, upTo: 250, rate: 0.2 }, { from: 250, upTo: Infinity, rate: 0.05 }]); break;
    case 'Outdoor Power Tools': fee = price <= 500 ? price * 0.15 : price * 0.08; break;
    case 'Personal Computers': case 'Personal Computers (Laptops)': fee = price * 0.06; break;
    case 'Tires & Wheels': fee = price * 0.1; break;
    case 'Watches':
      fee = tiered([{ from: 0, upTo: 1500, rate: 0.16 }, { from: 1500, upTo: Infinity, rate: 0.03 }]); break;
    default: fee = price * 0.15; break;
  }
...
function computeFBAFee(weightGrams: number): number {
...
function zeroResult(adsSpend: number): AmazonUSAResult {
  return {
    platform: 'Amazon USA', currency: 'USD',
...
export function calculateAmazonUSA(
  ...
  const {
    ...
    category = 'Everything Else', weight = 0, adsSpend = 0, returnRate = 0,
  } = product;

  // ── Guard: no sale, no fees ──────────────────────────────────────────────
  if (!sellingPrice || sellingPrice <= 0) return zeroResult(adsSpend);

  const fulfillmentMethod = settings.fulfillmentMethod || 'Self-Ship';
  ...
  const weightHandlingFee = 0, collectionFee = 0, codFee = 0, tcs = 0, gstOnFees = 0, otherFees = 0;
  const adsSpendVal = round2(adsSpend);

  const returnRateDecimal = returnRate / 100;
  const rtoShare = 0.05, cogsLossRate = 0.25;
  const reverseLogisticsPerReturn = isFBA ? fulfillmentFee * 0.5 : 3;
  ...
  return {
    platform: 'Amazon USA', currency: 'USD',
    sellingPrice: round2(sellingPrice), referralFee, closingFee, weightHandlingFee,
    fulfillmentFee, shippingFee, collectionFee, codFee, tcs, gstOnFees,
    adsSpend: adsSpendVal, returnLogisticsFee, returnImpact, otherFees,
