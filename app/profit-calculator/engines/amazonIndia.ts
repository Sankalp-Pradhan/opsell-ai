...
const REFERRAL_FEE_MAP: ReferralFeeMap = {
  ...
  MobilePhones: [{ max: Infinity, rate: 0.05 }],
  Laptops: [{ max: Infinity, rate: 0.06 }],
  Books: [
    { max: 250, rate: 0 },
    { max: 500, rate: 0.02 },
    { max: 1000, rate: 0.04 },
...
function getWeightHandlingFee(weightGrams: number, zone: ShippingZone): number {
  const zi = ({ Local: 0, Zonal: 1, National: 2 })[zone] ?? 2;
  const base = [[35, 42, 55], [47, 55, 72]];
  const extra = [8, 11, 15];
  if (weightGrams <= 500) return base[0][zi];
  if (weightGrams <= 1000) return base[1][zi];
  const additionalSlabs = Math.ceil((weightGrams - 1000) / 500);
  return base[1][zi] + additionalSlabs * extra[zi];
}

function getFbaFulfillmentFee(weightGrams: number): number {
  if (weightGrams <= 250) return 44;
  if (weightGrams <= 500) return 55;
  if (weightGrams <= 1000) return 90;
  if (weightGrams <= 2000) return 120;
  return 120 + Math.ceil((weightGrams - 2000) / 500) * 30;
}
...
function zeroResult(adsSpend: number): AmazonIndiaResult {
  return {
    platform: 'Amazon India', currency: 'INR',
...
export function calculateAmazonIndia(
  ...
  const {
    ...
    shippingZone = 'National', includeGSTAsFee = false,
  } = settings;
  ...
  const rtoShare = isCOD ? 0.3 : 0.1;
  const cogsLossRate = 0.25;

  const reverseLogisticsPerReturn =
    fulfillmentMethod === 'Platform Fulfillment' ? fulfillmentFee * 0.5 : weightHandlingFee * 0.5;
  const rtoPenaltyPerReturn =
    fulfillmentMethod === 'Platform Fulfillment' ? fulfillmentFee : weightHandlingFee;
...
export const AMAZON_INDIA_CATEGORIES = Object.keys(REFERRAL_FEE_MAP) as Array<keyof typeof REFERRAL_FEE_MAP>;